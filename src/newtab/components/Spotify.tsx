...
&code_challenge=${encodeURIComponent(CODE_CHALLENGE)}`
  ...
  async function loginPKCE() {
    try {
      console.log("login")
      const res: any = await chrome.identity.launchWebAuthFlow({
      ...
      let response: any = await fetch(
        spotify_token_fetch_url,
        spotify_token_fetch_options
      )
      response = await response.json()
      if (response.error) {
        throw new Error(`Error: getToken error`)
      }
  ...
  async function refreshPKCE(refresh_token) {
    try {
      ...
      let res: any = await fetch(
        ...
        spotify_token_refresh_options
      )
      res = await res.json()
      if (res.error) {
        throw new Error(`Error: refresh error`)
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        resolve([
          ...
          [dataSecond[0], dataSecond[1], dataSecond[2]]
        ])
      }
    })
  }
