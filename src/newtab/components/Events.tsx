...
function Events(props: Props) {
  ...
  async function getAndSetEvents() {
    try {
      ...
      const colorApiResponse = await fetch(
      ...
      if (curr) {
        setCurrentEvent({
          name: curr,
          color: events.items[0].colorId
            ? colors.event[events.items[0].colorId].background
            : "#3083FF"
        })
      }
      ...
      if (next) {
        setNextEvent({
          name: next,
          color: events.items[1].colorId
            ? colors.event[events.items[1].colorId].background
            : "#3083FF"
        })
      }
    } catch (err) {
  ...
  function getFormattedTimezoneOffset() {
    let offset: any = new Date(Date.now())
      .toString()
      .split("GMT")[1]
      .split(" ")[0]
    offset = offset.split("")
